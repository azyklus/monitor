...
{
   log::trace!("Got command `{}` by user `{}`", name, msg.author.name);
...
{
   match res {
      Ok(()) => log::trace!("Processed command `{}`", name),
      Err(why) => log::error!("Command `{}` returned error: {:?}", name, why),
   }
}
...
{
   log::error!("Could not find command called `{}`", name);
}

#[hook]
pub async fn normal(_ctx: &Context, msg: &Message)
{
   log::warn!("Message is not a command: `{}`", msg.content);
}
